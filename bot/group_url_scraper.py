...
REQUEST_DELAY_SECONDS = 10.0  # imkon qadar sekinroq - saytni zoriqtirmaslik ustuvor
...
async def sync_group_urls(start: int = None, end: int = None):
    ...
    for group_id in range(start, end + 1):
        # MUHIM: "Avtomatik skanerlashni to'xtatish" tugmasi ilgari
        # FAQAT yangi skanerlash boshlanishining oldini olardi - allaqachon
        # ishga tushgan skanerlash tugma bosilgandan keyin ham OXIRIGACHA
        # davom etaverar edi. Endi har bir ID oldidan tekshiramiz.
        if (await db.get_setting("scanning_enabled", "0")) != "1":
            logger.info(
                "Guruh havolalarini sinxronlash admin panel orqali to'xtatildi (ID %s da, %s/%s bajarilgan edi).",
                group_id, done_count, end - start + 1,
            )
            await db.set_setting("group_url_scan_status", "paused")
            return
        try:
