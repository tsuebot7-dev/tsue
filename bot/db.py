...
async def get_watched_group_names() -> list:
    """'Mening guruhim' sifatida kamida bitta foydalanuvchi tomonidan
    belgilangan barcha (takrorlanmas) guruh nomlari - almashtirishlarni
    kuzatish shu ro'yxat bo'yicha amalga oshiriladi (barcha 1290
    guruhni emas, faqat odamlar qiziqqanlarini)."""
    if USE_POSTGRES:
        async with _pg_pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT DISTINCT default_group_name, default_group_url FROM users "
