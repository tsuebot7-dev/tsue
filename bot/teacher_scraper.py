...
async def sync_teacher_urls(start: int = None, end: int = None):
    ...
    for teacher_id in range(start, end + 1):
        ...
        if (await db.get_setting("scanning_enabled", "0")) != "1":
            logger.info(
                "O'qituvchilar sinxronlashi admin panel orqali to'xtatildi (ID %s da, %s/%s bajarilgan edi).",
                teacher_id, done_count, end - start + 1,
            )
            await db.set_setting("teacher_scan_status", "paused")
            return
        try:
            url = build_teacher_url(teacher_id)
            svg_text = await fetch_svg_from_url(url)
